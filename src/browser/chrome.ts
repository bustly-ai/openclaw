...
export async function launchOpenClawChrome(
  ...
  if (needsDecorate) {
    try {
      ...
      log.info(`🦞 managed browser profile decorated (${profile.color})`);
    } catch (err) {
      log.warn(`managed browser profile decoration failed: ${String(err)}`);
    }
  }
  ...
  try {
    ...
    log.warn(`managed browser clean-exit prefs failed: ${String(err)}`);
  }
  ...
  log.info(
    `🦞 managed browser started (${exe.kind}) profile "${profile.name}" on 127.0.0.1:${profile.cdpPort} (pid ${pid})`,
  );
