...
type SupabaseClient = {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any -- Supabase query builders are highly dynamic and not all chain methods are used here.
...
function createSupabaseClient(state: BustlyOAuthState): SupabaseClient {
  ...
  try {
    supabaseModule = require("@supabase/supabase-js") as SupabaseModule;
  } catch (error) {
    throw new Error(
      "Missing @supabase/supabase-js dependency required for Bustly workspace bootstrap.",
      { cause: error },
    );
  }
  return supabaseModule.createClient(url, anonKey, {
    auth: {
