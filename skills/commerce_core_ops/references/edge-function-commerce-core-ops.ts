...
function normalizeShopifySinceField(value: unknown, fallback: "updated_at" | "processed_at" | "created_at"): "updated_at" | "processed_at" | "created_at" {
  const normalized = String(value || "").trim().toLowerCase();
...
function toShopifyOrderSortKey(sinceField: "updated_at" | "processed_at" | "created_at"): "UPDATED_AT" | "PROCESSED_AT" | "CREATED_AT" {
...
async function readShopifyEntity(
  ...
  const orderSinceDefault = request.entity === "orders" || request.entity === "order_items"
    ? "processed_at"
    : "updated_at";
  const orderSinceField = normalizeShopifySinceField(
    pickString(request.filters.since_field, request.filters.sinceField, request.filters.time_field, request.filters.timeField),
    orderSinceDefault,
  );
