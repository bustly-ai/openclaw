...
description: >-
  Multi-source trend analysis to build a decision-ready product shortlist. Use
  when user asks what to sell, trending products, or hot selling items.
status: enabled
layer: ecommerce
sub_layer: store-ops
---
...
- **Fast mode:** 3-5 searches, one shortlist, no deep charting.
- **Standard mode:** 8-15 searches with candidate ranking and one supporting chart if useful.
- **Deep mode:** full multi-round research, visual evidence, supplier view, and go / cautious / no-go decision.
- Let the user choose speed vs. depth when the request is broad.
...
- The team could take action immediately if Market Insight Product Selection returns a clear verdict, deliverable set, and next-step list.
